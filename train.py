...
def _main():
    ...
    parser.add_argument('--class_path', default='utils/class.txt', type=str,
    ...
    parser.add_argument(
        '--train_save_path', default='models/',
        help="The training model's checkpoint save path. The default path is 'model_weights/'.")
    parser.add_argument(
        '--model_name', default='modelnewishere.ckpt',
        help='The name that should be given to the checkpoint file. The default name is "modelnewisherelah.ckpt".')
    parser.add_argument(
    ...
    global_step = tf.compat.v1.get_variable(name='global_step',trainable=False,initializer=0,dtype=tf.int32)
    
    # returns a varlist containing only the vars of the conv layers right before the yolo layers
    trainable_var_list = tf.compat.v1.trainable_variables()
    last_layer_var_list = [i for i in trainable_var_list if i.shape[-1] == (5+num_classes)*num_anchors_per_detector] 
    train_op_with_frozen_variables = tf.compat.v1.train.AdamOptimizer(learning_rate=lr).minimize(loss, global_step=global_step, var_list=last_layer_var_list)
    train_op_with_all_variables = tf.compat.v1.train.AdamOptimizer(learning_rate=lr).minimize(loss, global_step=global_step, var_list=trainable_var_list)
